...
Modern software repositories contain hundreds of interconnected source files,source code, tests, documentation, configuration, and scripts. Understanding how these pieces relate to each other is essential as a matter of fact its crucial for technical onboarding, debugging, and impact analysis, yet existing tools present them in isolation. Regardless of the existing traditional tools used, Developers are going to struggle to gather the full picture of how a complete software system is structured.

CodeAtlas addresses this gap by transforming a codebase into an interactive knowledge graph where every entity and their relationships are explicit, explorable, and queryable. The system provides a unified interface that serves both as a navigation layer for developers and as a grounding layer for AI-assisted explanations, reducing the effort needed to form an accurate mental model of the repository.

We present the design of CodeAtlas, its graph schema, and its indexing methodology. An evaluation across repositories of varying scale examines graph construction quality, extraction accuracy, performance, and the effectiveness of graph-grounded AI assistance. With CodeAtlas there would be improvements in comprehension task efficiency compared to traditional file-level navigation.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\columnwidth]{images/scale_1.png}
    \caption{CodeAtlas system overview diagram.}
